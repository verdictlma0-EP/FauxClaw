...
Fauxclaw routes Claude Code requests through multiple **free** providers – AWS CodeWhisperer (Kiro), OpenRouter free tier, iFlow, NVIDIA NIM, Groq, and Gemini – with automatic failover, token refresh, and zero configuration beyond the initial setup.
...
NVIDIA NIM	Nemotron, Llama, Qwen, GLM	Free (40 req/min)	❌ Static key
Groq	Llama 3.3 70B, Mixtral, Gemma	Free tier	❌ Static key
Gemini	Gemini 2.0 Flash, Gemini 1.5 Pro	Free tier	❌ Static key
Fallback chain: Kiro → OpenRouter → iFlow → NVIDIA → Groq → Gemini
...
<div align="center"> <img src="assets/fauxclaw.png" alt="Fauxclaw in action" width="700"> <br> <em>Fauxclaw running in PowerShell</em> </div> ```
