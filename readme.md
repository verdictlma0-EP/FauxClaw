...
1. **Claude Code** sends a request to `http://localhost:8083/v1/messages`
2. **Fauxclaw** tries providers in order:
...
3. **If streaming** (Claude Code streams by default), Fauxclaw translates:
   - Kiro's binary AWS EventStream → Anthropic SSE
   - OpenRouter/iFlow SSE → pass through unchanged
4. **If a provider fails**, the circuit breaker opens for 60 seconds to avoid hammering it.
5. **Your Claude session** continues like nothing happened.
...
- [free-claude-code](https://github.com/Alishahryar1/free-claude-code) – Proxy architecture, model routing
- [9router](https://github.com/decolua/9router) – Kiro executor, token refresh
- [claudecodeui](https://github.com/siteboon/claudecodeui) – Session management patterns
